...
class MastodonService {
   ...
   * @param domain
   */
  static solveContext(current: StatusInterface, data: mastodon.v1.Context, domain: string) {
    let deque: string[] = []
    ...
    // store all lookups
    mapper.set(current?.getId(), current)
    flat.forEach((o) => {
      mapper.set(o.getId(), o)
    })
    ...
    while (deque.length > 0 && flat.length > 0) {
      ...
      const topI = mapper.get(top)

      // save list of children
      const matches = flat.filter((o) => o.getParentStatusId() === topI.getId())
      ...
      // reduce list
      flat = flat.filter((o) => o.getParentStatusId() !== mapper.get(top).getId())
    }
