...
import {
  ActivitypubStatusAdapter, ActivityPubUserAdapter, StatusInterface,
  UserInterface
...
class ChatroomService {
  static upsertConversation(db: Realm, {
    hash,
    me,
    conversation,
    domain,
    ...
    const usersI = conversation.accounts.map(
        (o) => ActivityPubUserAdapter(o, domain)
    )
    const conflict = ActivityPubConversationRepository.findById(db, conversation.id)

    db.write(() => {
      const savedMe = ActivityPubUserRepository.upsert(db, {user: me})
      const savedLatestStatus = ActivityPubStatusRepository.upsert(db,
          {status: latest, domain, subdomain})
      const savedServer = ActivityPubServerRepository.upsert(db, me.getInstanceUrl())
