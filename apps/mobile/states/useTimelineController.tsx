...
const TimelineControllerContext =
...
function WithTimelineControllerContext({children}: Props) {
  const [FetchMode, setFetchMode] = useState<TimelineFetchMode>(TimelineFetchMode.IDLE)

  function setTimelineTypeFn(x: TimelineFetchMode) {
    setFetchMode(x)
  }

  return <TimelineControllerContext.Provider value={{
