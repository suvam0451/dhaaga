...
const AppPaginationContext = createContext<Type>(defaultValue);
...
function WithAppPaginationContext({ children }: any) {
	...
	function append(data: StatusInterface[], keygen: Function) {
		const toAdd = [];
		for (let i = 0; i < data.length; i++) {
			const targetId = keygen ? keygen(data[i]) : data[i]?.getId();
